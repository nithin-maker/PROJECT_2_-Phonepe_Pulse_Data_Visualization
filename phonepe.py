...
select = st.sidebar.radio("", ["Home", "Data Exploration", "Top Charts"])
...
if select == "Home":
    ...
    # Right side content
    #st.write("## Welcome to PhonePe Data Visualization and Exploration!")

    st.write("### Overview of the Project")
    st.write("This project delves into data from PhonePe, India's premier digital payments platform. Using interactive visualizations, we reveal insights into transaction volumes, user demographics, and regional trends. Additionally, we analyze user behaviors, transaction types, and popular payment methods. Our aim is to offer actionable insights to businesses and analysts, empowering them to make informed decisions in the dynamic landscape of digital finance")

    st.write("### Insights")
    st.write("- Utilizing data aggregation techniques to summarize transaction, user, and insurance data.")
    st.write("- Visualizing geographical distribution of transactions using maps.")
    st.write("- Identifying top brands of mobiles used by PhonePe users.")
    st.write("- Analyzing transaction amounts and counts at state and district levels.")
    st.write("- Presenting top charts and rankings based on various metrics.")
    st.write("- Uncovering trends in transaction frequency and amounts over time.")
    st.write("- Exploring user engagement metrics and identifying patterns.")


    st.markdown("---")
    
    st.write("### Start Exploring Now!")
    st.write("Ready to unlock the power of PhonePe data?")
    st.write("Choose 'Data Exploration' from the menu and embark on a journey of discovery.")
...
if select == "Data Exploration":
    ...
    with tab1:
        ...
        if method == "Insurance Analysis":
            col1, col2 = st.columns(2)
            with col1:
                years = st.selectbox("**Select the Year**", options=range(Aggre_insurance["Years"].min(), Aggre_insurance["Years"].max() + 1))

            df_agg_insur_Y = Aggre_insurance_Y(Aggre_insurance, years)

            col1, col2 = st.columns(2)
            with col1:
                quarters = st.selectbox("**Select the Quarter**", options=df_agg_insur_Y["Quarter"].unique())
            ...
            col1, col2 = st.columns(2)
            with col1:
                years_at = st.selectbox("**Select the Year**", options=range(Aggre_transaction["Years"].min(), Aggre_transaction["Years"].max() + 1))

            df_agg_tran_Y = Aggre_insurance_Y(Aggre_transaction, years_at)

            col1, col2 = st.columns(2)
            with col1:
                quarters_at = st.selectbox("**Select the Quarter**", options=df_agg_tran_Y["Quarter"].unique())

            df_agg_tran_Y_Q = Aggre_insurance_Y_Q(df_agg_tran_Y, quarters_at)
            
            state_Y_Q = st.selectbox("**Select the State**", options=df_agg_tran_Y_Q["States"].unique())

            Aggre_Transaction_type(df_agg_tran_Y_Q, state_Y_Q)

        elif method == "User Analysis":
            year_au = st.selectbox("Select the Year_AU", options=Aggre_user["Years"].unique())
            agg_user_Y = Aggre_user_plot_1(Aggre_user, year_au)

            quarter_au = st.selectbox("Select the Quarter_AU", options=agg_user_Y["Quarter"].unique())
            agg_user_Y_Q = Aggre_user_plot_2(agg_user_Y, quarter_au)

            state_au = st.selectbox("**Select the State_AU**", options=agg_user_Y["States"].unique())
